...
Order status is stored as a VARCHAR(20) in PostgreSQL and validated at the application layer using a Python Enum (OrderStatus with pending, completed, cancelled). This provides type safety while allowing new business statuses (e.g., efunded, partially_fulfilled) to be introduced in the future without requiring complex PostgreSQL enum type migrations.
