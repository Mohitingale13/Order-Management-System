...
- Scaffolded the frontend using Vite:
  ...
  `
- Installed all required npm dependencies (npm install).
...
- Created root .gitignore ensuring that neither local virtual environments (backend/.venv/), secrets (.env, backend/.env), node modules (frontend/nnode_modules/), nor build artifacts (dist/) can be accidentally committed to source control.
